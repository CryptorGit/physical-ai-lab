...
# Frozen simulation envelope from the exp_004 perturbation acceptance.  The
# reverse cap is deliberately asymmetric: -0.08 exceeded the primary-error
# threshold, while -0.075 passed the 20x30 perturbation matrix.
DEFAULT_COMMAND_MIN: Final = (-0.075, -0.16, -1.00)
...
class SafeGaitRouter:
    ...
    def _atomic_target(self, clipped: np.ndarray) -> str | None:
        ...
        if vx > -linear_enter or abs(vy) > lateral_exit or abs(yaw) < yaw_enter:
            return None
