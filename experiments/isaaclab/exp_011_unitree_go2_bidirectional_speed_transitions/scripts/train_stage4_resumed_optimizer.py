"""Run Stage 4 with the official checkpoint Adam state strictly restored."""
...
def save_checkpoint(runner, path, local_iteration):
    ...
    payload["iter"] = 999 + local_iteration
    payload["infos"] = {
        "source_checkpoint_iteration": 999,
        "stage4_local_iteration": local_iteration,
...
def main():
    ...
    agent_cfg.max_iterations = 300
    if args.device:
        cfg.sim.device = args.device
        agent_cfg.device = args.device
    raw = gym.make("Isaac-Exp011-Go2-Bidirectional-0To2-v0", cfg=cfg)
    ...
    optimizer_checks = {
        ...
        "adam_step_20000": step_min == step_max == 20000,
        "learning_rate_exact": runner.alg.learning_rate == 0.0003901844231062339,
        "first_moment_match": abs(first - parent_first) <= 1e-12,
        "second_moment_match": abs(second - parent_second) <= 1e-12,
        "source_iteration_999": runner.current_learning_iteration == 999,
    }
    resume_audit = {
        ...
        "stage4_local_iteration": 0, "state_count": len(loaded_opt["state"]),
    ...
    dump(args.output / "preupdate_model_identity.json", identity)
    ...
    manifest = [{
        ...
        "local_iteration": 0, "source_iteration": 999,
        ...
        "curriculum_hash": sha(
            REPO / "results/exp_011_unitree_go2_bidirectional_speed_transitions/"
            "stage2_continuous_0_to_2_training/command_curriculum_config.json"
        ),
        "reward_hash": sha(
            REPO / "results/exp_011_unitree_go2_bidirectional_speed_transitions/"
            "stage2_continuous_0_to_2_training/stage2_reward_config.json"
        ),
    }]
    dump(args.output / "checkpoint_manifest.json", {"status": "TRAINING", "checkpoints": manifest})
    curves, early_rows = [], []
    save_points = {1, 10, 25, 50, 75, 100, 150, 200, 250, 300}
    fixed_obs = obs
    consecutive_kl = 0
    start_time = time.time()
    runner.alg.train_mode()
    for local_iteration in range(1, 301):
        ...
        with torch.inference_mode():
            for _ in range(agent_cfg.num_steps_per_env):
                ...
                for cohort_id in range(4):
                    rewards_by_cohort[cohort_id].append(float(rewards[cohort_id::4].mean()))
        ...
        cohort = torch.arange(2048, device=device).remainder(4).repeat(agent_cfg.num_steps_per_env)
        ...
        if current_rollout is not None:
            ...
            reference = (
                torch.load(STAGE3_BATCH, map_location="cpu", weights_only=False)
                if STAGE3_BATCH.exists() else None
            )
            dump(
        ...
        row = {
            ...
            "source_iteration": 999 + local_iteration,
            "interaction_count": local_iteration * 2048 * agent_cfg.num_steps_per_env,
            ...
            "fall_rate_in_rollout": dones_total / (2048 * agent_cfg.num_steps_per_env),
            "reward_mean": sum(sum(values) for values in rewards_by_cohort) / (4 * agent_cfg.num_steps_per_env),
            "reward_zero": sum(rewards_by_cohort[0]) / len(rewards_by_cohort[0]),
            "reward_steady": sum(rewards_by_cohort[1]) / len(rewards_by_cohort[1]),
            "reward_acceleration": sum(rewards_by_cohort[2]) / len(rewards_by_cohort[2]),
            "reward_deceleration": sum(rewards_by_cohort[3]) / len(rewards_by_cohort[3]),
        ...
        if local_iteration == 1:
            dump(args.output / "first_update_causal_confirmation.json", {
                ...
                "stage2": {"reported_kl": 0.5129385441541672, "exact_kl": 0.5098640322685242, "clip_fraction": 0.7820231318473816},
                "stage3_shadow": {"exact_kl": 0.013813133351504803, "clip_fraction": 0.1928914487361908},
                "stage4_pilot": row, "gate_checks": first_gate,
        ...
        if local_iteration in save_points:
            ...
            manifest.append({
                ...
                "local_iteration": local_iteration, "source_iteration": 999 + local_iteration,
                "actor_hash": parameter_hash(runner.alg.actor), "critic_hash": parameter_hash(runner.alg.critic),
        ...
        dump(args.output / "optimization_stability.json", {
            "status": "TRAINING" if local_iteration < 300 else "PASS",
            "iterations_completed": local_iteration,
            "interactions": local_iteration * 2048 * agent_cfg.num_steps_per_env,
        ...
        print(
            f"STAGE4 local={local_iteration}/300 exact_kl={exact_kl:.5f} "
