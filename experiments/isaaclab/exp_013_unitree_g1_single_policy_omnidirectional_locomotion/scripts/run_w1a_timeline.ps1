...
foreach ($iteration in @("initial", "1", "10", "20", "40", "60", "80", "100", "120", "140", "160", "180", "200")) {
    & $python $evaluator --suite timeline --checkpoint (Join-Path $checkpointDir "model_$iteration.pt") --tag "iter_$iteration" --headless
    if ($LASTEXITCODE -ne 0) { throw "Timeline evaluation failed at $iteration" }
}
