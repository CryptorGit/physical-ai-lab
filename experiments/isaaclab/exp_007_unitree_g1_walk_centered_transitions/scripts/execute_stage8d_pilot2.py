"""Execute the single authorized Stage 8D RUN_TO_WALK acquisition Pilot 2."""
...
REPO = EXP.parents[2]
OUT = REPO / "results/exp_007_unitree_g1_walk_centered_transitions/stage8d_run_to_walk_pilot2_walk_acquisition"
CFG_PATH = EXP / "configs/stage8d_run_to_walk_pilot2_walk_acquisition.yaml"
PILOT1_CFG_PATH = EXP / "configs/stage8b_run_to_walk_pilot1.yaml"
SEAL = EXP / "configs/stage8d_run_to_walk_pilot2_walk_acquisition.sha256"
...
PARENT = REPO / "results/exp_007_unitree_g1_walk_centered_transitions/stage8c_run_to_walk_pilot1_execution/checkpoints/model_10.pt"
ALLOWED_HEAD = "0362aa5bb9f7b6a27ec66b7afeafc1290b8770e3"
CONFIG_SHA = "3cf7513d68730637dbc9652b526a7ec6b31c976ec986482624615e55f383f324"
REWARD_SHA = "a1b23b3ed64002e0b915336480ed0071e168247253ff9241cdeb14e4338af00e"
ACTOR_SHA = "bfccc2fb9d066b28d24773ebb950e72319a9fea9c8464a736c6de016b5e52db7"
PARENT_SHA = "f54ead0da2a192e238e1fd6dbcb48670fb785f7ef7e7766c64d0dfbf06eba263"
RUN_NAME = "stage8d-pilot2-walkacq3cf7513d-seed20270115"
EXPECTED = {
...
def actor_initialization_digest(cfg):
    return digest({
...
def authorize(cfg):
    ...
    pilot1 = yaml.safe_load(PILOT1_CFG_PATH.read_text(encoding="utf-8"))
    old_flat, new_flat = flattened(pilot1), flattened(cfg)
    differences = {
    ...
    write_json("pilot1_vs_pilot2_config_diff.json", {
        ...
        "walk_acquisition_weight_multiplier": cfg["reward"]["walk_contract_progress"] / pilot1["reward"]["walk_contract_progress"],
    })
    checks = {
        ...
        "actor_initialization_sha": actor_initialization_digest(cfg) == ACTOR_SHA,
        "parent_checkpoint_sha": file_sha(PARENT) == PARENT_SHA == cfg["actor"]["parent_sha256"],
        "config_single_reward_delta": not unexpected and cfg["reward"]["walk_contract_progress"] == 2.0 * pilot1["reward"]["walk_contract_progress"],
        "freeze_ready": freeze["status"] == "FROZEN_READY_FOR_PILOT1",
        "protected_hashes": all(actual[key] == EXPECTED[key][1] for key in EXPECTED),
        "expected_run_name": RUN_NAME == f"stage8d-pilot2-walkacq{CONFIG_SHA[:8]}-seed{cfg['experiment']['training_seed']}",
    ...
    if not result["authorized"]:
        raise RuntimeError(f"Stage 8D authorization denied: {result}")
    return actual
...
def main():
    ...
    (OUT / "pilot2_config.yaml").parent.mkdir(parents=True, exist_ok=True)
    (OUT / "pilot2_config.yaml").write_text(CFG_PATH.read_text(encoding="utf-8"), encoding="utf-8")
    write_json("stage8c_reference.json", {
        "classification": "NO_LEARNING_SIGNAL",
        "selected_checkpoint": str(PARENT.relative_to(REPO)).replace("\\", "/"),
        "selected_checkpoint_sha256": PARENT_SHA,
        "failure_class": "WALK_BASIN_RETENTION_FAILURE",
        "results_modified": False,
    })
    write_json("pilot2_protocol_hashes.json", {
        "pilot2_config_sha256": CONFIG_SHA,
        "pilot2_reward_sha256": REWARD_SHA,
        "actor_initialization_sha256": ACTOR_SHA,
        "pilot2_parent_sha256": PARENT_SHA,
        "expected_run_name": RUN_NAME,
    })

    curves, reward_rows, source_rows, phase_rows, routing_rows, timing_rows, streak_rows, manifests, evaluation_rows, episode_rows = [], [], [], [], [], [], [], [], [], []
    failures = Counter()
    abort = None
    ...
    with launch_simulation(task_cfg, args):
        ...
        actor.load_state_dict(parent_payload["actor"], strict=True)
        ...
        initial_bitwise = tensor_sha(actor.state_dict()) == tensor_sha(parent_payload["actor"])
        ...
        preflight_checks = {
            ...
            "reward_sha": digest({"weights": cfg["reward"], "thresholds": cfg["reward_thresholds"], "completion": cfg["completion"]}) == REWARD_SHA,
            "actor_initialization_sha": actor_initialization_digest(cfg) == ACTOR_SHA,
            "parent_sha": file_sha(PARENT) == PARENT_SHA,
            "parent_actor_bitwise": initial_bitwise, "observation_152": True, "action_37": True,
            ...
            "exploration_settings_unchanged": cfg["exploration"]["initial_std"] == 0.25 and cfg["exploration"]["abort_min"] == 0.05 and cfg["exploration"]["abort_max"] == 0.75,
            "optimizer_groups_exclude_frozen": all(p.requires_grad for p in trainable),
            "expected_run_name": RUN_NAME == "stage8d-pilot2-walkacq3cf7513d-seed20270115",
        }
        ...
        def rollout(state, deterministic, training, collect_details=False):
            ...
            walk_valid_streak = torch.zeros(cohort, dtype=torch.long, device=device)
            max_walk_valid_streak = torch.zeros(cohort, dtype=torch.long, device=device)
            walk_progress_raw_fires = torch.zeros(cohort, dtype=torch.long, device=device)
            walk_progress_raw_sum = torch.zeros(cohort, device=device)
            walk_progress_weighted_sum = torch.zeros(cohort, device=device)
            break_counts = {name: torch.zeros(cohort, dtype=torch.long, device=device) for name in ("speed", "heading", "contact", "flight", "safety")}
            detail_rows = []
            ...
            for step in range(cfg["rollout"]["horizon_steps"]):
                ...
                contact_ok = walk_contact
                flight_ok = ~excessive
                safety_ok = ~fall & ~reverse & ~safety
                contract_good = speed_ok & heading_ok & cycle_terminated & contact_ok & flight_ok & safety_ok
                previous_hold = walk_hold.clone()
                previous_streak = walk_valid_streak.clone()
                walk_hold = torch.where(contract_good & active, walk_hold + dt, torch.zeros_like(walk_hold))
                walk_valid_streak = torch.where(contract_good & active, walk_valid_streak + 1, torch.zeros_like(walk_valid_streak))
                max_walk_valid_streak = torch.maximum(max_walk_valid_streak, walk_valid_streak)
                streak_broke = (previous_streak > 0) & ~contract_good & active
                unresolved = streak_broke.clone()
                for reason, failed in (
                    ...
                    hit = unresolved & failed
                    break_counts[reason] += hit.long()
                    unresolved &= ~hit
                ...
                progress_raw = raw["walk_contract_progress"] * active_before.float()
                progress_weighted = weighted["walk_contract_progress"] * active_before.float()
                walk_progress_raw_fires += (progress_raw > 0).long()
                walk_progress_raw_sum += progress_raw
                walk_progress_weighted_sum += progress_weighted
                if collect_details:
                    active_ids = torch.nonzero(active_before).flatten().cpu().tolist()
                    for local in active_ids:
                        primary = ""
                        if bool(streak_broke[local]):
                            if not bool(safety_ok[local]): primary = "safety"
                            elif not bool(flight_ok[local]): primary = "flight"
                            elif not bool(contact_ok[local]): primary = "contact"
                            elif not bool(heading_ok[local]): primary = "heading"
                            elif not bool(speed_ok[local]): primary = "speed"
                        detail_rows.append({
                            "cohort_local_index": local,
                            "physical_env_id": int(selected[local]),
                            "source_speed_mps": float(state.source_speed[selected][local]),
                            "transition_step": step,
                            "time_s": float(elapsed[local]),
                            "walk_compatible_contact": bool(contact_ok[local]),
                            "speed_condition": bool(speed_ok[local]),
                            "heading_condition": bool(heading_ok[local]),
                            "contact_condition": bool(contact_ok[local]),
                            "flight_condition": bool(flight_ok[local]),
                            "safety_condition": bool(safety_ok[local]),
                            "walk_valid": bool(contract_good[local]),
                            "consecutive_walk_valid_steps": int(walk_valid_streak[local]),
                            "required_steps": 20,
                            "streak_broke": bool(streak_broke[local]),
                            "break_condition": primary,
                            "speed_error_mps": float((speed[local] - 1.2).abs()),
                            "heading_error_rad": float(heading_error[local].abs()),
                            "excessive_flight": bool(excessive[local]),
                            "vertical_velocity_mps": float(vertical[local]),
                            "lateral_velocity_mps": float(lateral[local]),
                            "slip_failure": bool((slip_dwell >= 0.2)[local]),
                            "saturation_failure": bool(((ankle_dwell >= 0.2) | (knee_dwell >= 0.2))[local]),
                            "action_rate": float(action_rate[local]),
                            "target_walk_action_difference": float(torch.sqrt((action.detach()[local] - walk_mean[local]).square().mean())),
                            "walk_acquisition_progress_raw": float(progress_raw[local]),
                            "walk_acquisition_progress_weighted_parent": float(progress_raw[local] * 2.0),
                            "walk_acquisition_progress_weighted": float(progress_weighted[local]),
                        })
            ...
            result = {
                ...
                "walk_valid_streak": walk_valid_streak, "max_walk_valid_streak": max_walk_valid_streak,
                "walk_progress_raw_fires": walk_progress_raw_fires,
                "walk_progress_raw_sum": walk_progress_raw_sum,
                "walk_progress_weighted_sum": walk_progress_weighted_sum,
                "break_counts": break_counts, "detail_rows": detail_rows,
            }
            return result

        def evaluate_checkpoint(label, checkpoint_path, seed, episodes_per_source=20, collect_details=False):
            ...
            total_episodes = 2 * episodes_per_source
            source = prepare_source(seed, total_episodes, balanced=True)
            if source is None:
                raise RuntimeError(f"evaluation source preparation failed: {label}")
            result = rollout(source, deterministic=True, training=False, collect_details=collect_details)
            ...
            heading_max = torch.zeros(total_episodes, device=device)
            for _ in range(round(5.0 / dt)):
                ...
                command = MotionCommand(torch.full((total_episodes,), 1.2, device=device), source.heading[selected], target_yaw_rate_radps=(0.8 * heading_error).clamp(-0.3, 0.3))
                with torch.no_grad():
            ...
            for speed_value in (2.6, 2.8):
                ...
                row = {
                    ...
                    "max_walk_valid_streak_mean": float(result["max_walk_valid_streak"][mask].float().mean()),
                    "max_walk_valid_streak_p95": float(torch.quantile(result["max_walk_valid_streak"][mask].float(), 0.95)),
                    "max_walk_valid_streak_max": int(result["max_walk_valid_streak"][mask].max()),
                    "walk_progress_fire_episode_rate": float((result["walk_progress_raw_fires"][mask] > 0).float().mean()),
                }
                evaluation_rows.append(row)
                for local in torch.nonzero(mask).flatten().tolist():
                    episode_rows.append({
                        "checkpoint": label, "source_speed_mps": speed_value, "episode": local,
                        **{name: bool(value[local]) for name, value in result["outcomes"].items()},
                        "walk_hold": bool(hold_ok[local]), "full_edge": bool(full_edge[local]),
                        "phase": int(result["source_phase"][local]), "duration": float(result["elapsed"][local]),
                        "max_walk_valid_streak": int(result["max_walk_valid_streak"][local]),
                        "walk_progress_raw_fires": int(result["walk_progress_raw_fires"][local]),
                        "walk_progress_raw_sum": float(result["walk_progress_raw_sum"][local]),
                        "walk_progress_weighted_sum": float(result["walk_progress_weighted_sum"][local]),
                    })
            return result

        # Initial deterministic baseline precedes every optimizer update.
        initial_result = evaluate_checkpoint("initial", OUT / "checkpoints/initial.pt", 20270215, episodes_per_source=40, collect_details=True)
        ...
        write_json("initial_baseline_summary.json", {"episodes": 80, "episodes_per_source": 40, "per_source": initial_per})
        write_csv("walk_contract_streaks.csv", initial_result["detail_rows"])
        streaks = initial_result["max_walk_valid_streak"].float()
        raw_fires = initial_result["walk_progress_raw_fires"].float()
        near_contract = streaks > 0
        fire_episode_rate = float((raw_fires[near_contract] > 0).float().mean()) if bool(near_contract.any()) else 0.0
        if streaks.std() > 0 and raw_fires.std() > 0:
            correlation = float(torch.corrcoef(torch.stack((streaks, raw_fires)))[0, 1])
        else:
            correlation = 0.0
        break_totals = {name: int(value.sum()) for name, value in initial_result["break_counts"].items()}
        write_json("walk_contract_break_conditions.json", {
            "primary_failure_condition_counts": break_totals,
            "priority": ["safety", "flight", "contact", "heading", "speed"],
            "required_consecutive_steps": 20,
        })
        write_json("walk_acquisition_root_cause.json", {
            "failure_class": "WALK_BASIN_RETENTION_FAILURE",
            "episodes": 80,
            "max_walk_valid_streak": {
                "mean": float(streaks.mean()),
                "p50": float(torch.quantile(streaks, 0.50)),
                "p95": float(torch.quantile(streaks, 0.95)),
                "max": int(streaks.max()),
                "required": 20,
            },
            "break_conditions": break_totals,
            "walk_acquisition_raw_fire_episode_rate_near_contract": fire_episode_rate,
            "raw_fire_to_max_streak_correlation": correlation,
        })
        reachability_pass = fire_episode_rate >= 0.50 and correlation > 0.0
        parent_reproduced = all(
            row["run_cycle_termination"] >= 0.90
            and row["walk_contact_acquisition"] >= 0.90
            and row["walk_contract_acquisition"] < 0.20
            and row["transition_completion"] < 0.20
            and max(row["fall"], row["slip"], row["impact"], row["saturation"], row["reverse_failure"]) <= 0.05
            for row in initial_per
        )
        write_json("walk_acquisition_reward_reachability.json", {
            "status": "PASS" if reachability_pass else "WALK_ACQUISITION_REWARD_NOT_REACHABLE",
            "near_contract_definition": "maximum consecutive WALK-valid steps > 0",
            "near_contract_episodes": int(near_contract.sum()),
            "near_contract_fire_episode_rate": fire_episode_rate,
            "minimum_required_fire_rate": 0.50,
            "raw_fire_to_max_streak_correlation": correlation,
            "positive_correlation_required": True,
            "formula_changed": False,
        })
        write_json("pilot_execution_preflight.json", {
            "status": "PASS" if all(preflight_checks.values()) and reachability_pass and parent_reproduced else "FAIL",
            "checks": {
                **preflight_checks,
                "walk_acquisition_reward_reachable": reachability_pass,
                "parent_baseline_reproduced": parent_reproduced,
            },
            "run_name": RUN_NAME,
        })
        if not reachability_pass:
            raise RuntimeError("WALK_ACQUISITION_REWARD_NOT_REACHABLE")
        if not parent_reproduced:
            raise RuntimeError("PARENT_BASELINE_NOT_REPRODUCED")
        ...
        for iteration in range(1, cfg["experiment"]["iterations"] + 1):
            ...
            curves.append({
                ...
                **outcome_rates, "policy_loss": mean_loss[0], "value_loss": mean_loss[1], "entropy": mean_loss[2],
                ...
                "max_walk_valid_streak_mean": float(result["max_walk_valid_streak"].float().mean()),
                "max_walk_valid_streak_p95": float(torch.quantile(result["max_walk_valid_streak"].float(), 0.95)),
                "max_walk_valid_streak_max": int(result["max_walk_valid_streak"].max()),
                "walk_streak_ge_5_rate": float((result["max_walk_valid_streak"] >= 5).float().mean()),
                "walk_streak_ge_10_rate": float((result["max_walk_valid_streak"] >= 10).float().mean()),
                "walk_streak_ge_15_rate": float((result["max_walk_valid_streak"] >= 15).float().mean()),
                "walk_streak_ge_20_rate": float((result["max_walk_valid_streak"] >= 20).float().mean()),
                "walk_acquisition_progress_raw_fires": int(result["walk_progress_raw_fires"].sum()),
                "walk_acquisition_progress_weighted_contribution": float(result["walk_progress_weighted_sum"].sum()),
            })
            ...
            streak_rows.append({
                "iteration": iteration,
                "mean": float(result["max_walk_valid_streak"].float().mean()),
                "p95": float(torch.quantile(result["max_walk_valid_streak"].float(), 0.95)),
                "max": int(result["max_walk_valid_streak"].max()),
                "ge_5": int((result["max_walk_valid_streak"] >= 5).sum()),
                "ge_10": int((result["max_walk_valid_streak"] >= 10).sum()),
                "ge_15": int((result["max_walk_valid_streak"] >= 15).sum()),
                "ge_20": int((result["max_walk_valid_streak"] >= 20).sum()),
                "raw_fires": int(result["walk_progress_raw_fires"].sum()),
                "weighted_contribution": float(result["walk_progress_weighted_sum"].sum()),
                **{f"break_{name}": int(value.sum()) for name, value in result["break_counts"].items()},
            })
            ...
            write_csv("walk_contract_streak_statistics.csv", streak_rows)
            ...
            print(f"[Stage8D] iteration={iteration:03d} completion={outcome_rates['completion']:.3f} timeout={outcome_rates['timeout']:.3f} streak_p95={float(torch.quantile(result['max_walk_valid_streak'].float(), 0.95)):.1f} kl={sum(kls)/len(kls):.5f} std={float(std.mean()):.4f}", flush=True)
        ...
        if abort is None:
            ...
            for index, (label, name) in enumerate(checkpoint_names):
                evaluate_checkpoint(label, OUT / "checkpoints" / name, 20270315 + index)
    ...
    if abort:
        failures[abort] += 1
        raise RuntimeError(f"Stage 8D aborted: {abort}")
